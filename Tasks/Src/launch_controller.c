...
static void begin_rx(void)
{
    ...
    if (HAL_UARTEx_ReceiveToIdle_DMA(launch_uart, uart_rx, sizeof(uart_rx)) == HAL_OK && launch_uart->hdmarx != 0) __HAL_DMA_DISABLE_IT(launch_uart->hdmarx, DMA_IT_HT);
}
...
{
    launch_uart = uart; line_length = 0U; LaunchMotorBus_Init(can1, can2); begin_rx();
}
...
void LaunchController_UartError(UART_HandleTypeDef *huart) { if (huart == launch_uart) { line_length = 0U; vofa_tx_busy = 0U; (void)HAL_UART_AbortReceive(huart); begin_rx(); } }
...
void LaunchController_Task(void *argument)
{
    ...
    for (;;) {
        ...
        if (active != 0U) {
            ...
            if ((jam_retreat_active == 0U) && (left != 0) && (left->online != 0U)) { actual_left_rpm = (float)left->speed_rpm; LaunchMotorBus_SetCommand(LAUNCH_LEFT_FLYWHEEL_CAN, LAUNCH_LEFT_FLYWHEEL_ID, pid_update(&left_pid, LAUNCH_LEFT_FLYWHEEL_DIR * command.speed_rpm, actual_left_rpm, LAUNCH_M3508_FILTER_ALPHA, LAUNCH_M3508_CURRENT_LIMIT)); }
            if ((jam_retreat_active == 0U) && (right != 0) && (right->online != 0U)) { actual_right_rpm = (float)right->speed_rpm; LaunchMotorBus_SetCommand(LAUNCH_RIGHT_FLYWHEEL_CAN, LAUNCH_RIGHT_FLYWHEEL_ID, pid_update(&right_pid, LAUNCH_RIGHT_FLYWHEEL_DIR * command.speed_rpm, actual_right_rpm, LAUNCH_M3508_FILTER_ALPHA, LAUNCH_M3508_CURRENT_LIMIT)); }
...
#endif
                    if ((flywheel_ready != 0U) && (now - start_ms >= LAUNCH_FLYWHEEL_SPINUP_MS) &&
                        (fire_planner.issued_count >= command.bullet_count) &&
                        (completed_bullets >= command.bullet_count)) { active = 0U; set_all_zero(); }
                }
            }
        }
        else set_all_zero();
        LaunchMotorBus_Service(now); osDelay(LAUNCH_CONTROL_PERIOD_MS);
    }
}
...
void LaunchController_VofaTask(void *argument)
{
    ...
    for (;;) {
        ...
        out[4] = (float)completed_bullets; out[5] = actual_left_rpm; out[6] = feeder_target_deg; out[7] = feeder_deg;
        ...
        if (vofa_tx_busy == 0U)
        {
            ...
            memcpy(&vofa_tx_buffer[VOFA_CHANNEL_COUNT * sizeof(float)], tail,
                   sizeof(tail));
            vofa_tx_busy = 1U;
            if (HAL_UART_Transmit_DMA(launch_uart, vofa_tx_buffer,
                                      sizeof(vofa_tx_buffer)) != HAL_OK)
                vofa_tx_busy = 0U;
        }
        osDelay(LAUNCH_VOFA_PERIOD_MS);
    }
}
